...
link to rcos blogpost:

https://rcos.io/projects/brianhotopp/fastfoodatabase/blog

**Checkpoint 1:**

mongodb server status:

![image-20210415184635103](./image-20210415184635103.png)

mongodb connection:

![image-20210415184608208](./image-20210415184608208.png)

**Checkpoint 2:**

Create collections message:

![image-20210415184923286](./image-20210415184923286.png)

**Checkpoint 3:**

Finding entered and modified records:

![image-20210415191720673](./image-20210415191720673.png)

Result of the git diff:

![image-20210415192014892](./image-20210415192014892.png)
...
if __name__ == '__main__':
    ...
    res = db.definitions.find({})
    for x in res:
        pprint(x)
    # fetch one record
    print("Find one record")
    res = db.definitions.find_one()
    for x in res:
        pprint(x)
    # fetch a specific record
    print("Fetch a specific record")
    res = db.definitions.find({"word": "Capitaland"})
    for x in res:
        pprint(x)

    # fetch a record by id
    print("Fetch a record by id")
    res = db.definitions.find({"_id": ObjectId("56fe9e22bad6b23cde07b8c3")})
    for x in res:
        pprint(x)

    # insert a record
    print("Inserting a record")
    res = db.definitions.insert_one({"word": "python inserted word"})
    pprint(res)
```
...
{'_id': ObjectId('56fe9e22bad6b23cde07b942'),
 ...
 'word': 'Whoosh'}
{'_id': ObjectId('6078d04ef712088fda210391'), 'word': 'python inserted word'}
{'_id': ObjectId('6078d0675de921d3b4ec346f'), 'word': 'python inserted word'}
Find one record
'_id'
'definition'
'word'
...
{'_id': ObjectId('56fe9e22bad6b23cde07b8c3'),
 ...
 'word': 'Blast'}
Inserting a record
<pymongo.results.InsertOneResult object at 0x7fcafc0b3b40>
...
def random_word_requester():
    ...
    res = list(db.definitions.find({}))
    word = random.choice(res)
    newtime = datetime.datetime.utcnow().isoformat()
    definitions.update_one({'word': word['word']}, {'$push': {'dates': newtime}})
...
![image-20210415214522537](./image-20210415214522537.png)