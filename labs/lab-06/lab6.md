...
Modified python code to test custom length 5 words:
...
Modified python code to test custom length 4 words:
...
def generate_graph(words):
    ...
    def edit_distance_one(word):
        all_possible = itertools.permutations(word)
        for a_word in all_possible:
            possible = ''.join(a_word)
            for i in range(len(possible)):
                left, c, right = possible[0:i], possible[i], possible[i + 1:]
                j = lookup[c]  # lowercase.index(c)
                for cc in lowercase[j + 1:]:
                    yield left + cc + right
    candgen = ((word, cand) for word in sorted(words)
